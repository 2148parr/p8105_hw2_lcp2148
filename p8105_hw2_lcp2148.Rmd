...
Key variables in Mr Trash Wheel include `r names(mr_trash)`. `print(mr_trash)` The number of observations in this data set is 456. The median number of sports balls in a dumpster in 2019 `r mr_trash %>% filter(year == "2019") %>% median(sports_balls)` 

For the precipitation data set, the variables are `r names(precipfinal)`. Total precipitation in 2018 was `r precipfinal %>% filter(year == "2018") %>% sum(precip_in)` in. 
...
The `pols_month` data set contains data on the party affiliation of the current president and number of republican and democratic governors, senators, and senators on specific dates, which were changed to indicate year and month. The dimension of the merged data set, `merged_538`, is `r dim(merged_538)`. The range of years in this merged data set is `r merged_538 %>% select(year) %>% range`. The names of key variables are month and year, out of the total variables in the data set, `r merged_538 %>% names`. 
